...
class PreprocessorConfiguration {
    constructor(explicitValues, environmentOverrides, cypressConfiguration) {
    ...
    get stepDefinitions() {
        ...
        if ("specPattern" in config) {
            return exports.DEFAULT_POST_10_STEP_DEFINITIONS;
        }
        else {
            return exports.DEFAULT_PRE_10_STEP_DEFINITIONS.map((pattern) => pattern.replace("[integration-directory]", (0, helpers_1.ensureIsRelative)(config.projectRoot, config.integrationFolder)));
        }
    }
    get messages() {
...
async function resolve(cypressConfig, environment, configurationFileResolver = cosmiconfigResolver) {
    ...
    if (result) {
        ...
        return new PreprocessorConfiguration(config, environmentOverrides, cypressConfig);
    }
    else {
        (0, debug_1.default)("resolved no configuration");
        return new PreprocessorConfiguration({}, environmentOverrides, cypressConfig);
    }
}
